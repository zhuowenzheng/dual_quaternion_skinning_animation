...
vector<vector<BoneTransform>> boneTransforms;
vector<vector<glm::mat4>> boneMatrices;
...
void render()
{
	...
	// Draw character
	double fps = 30;
	int frameCount = 27; // TODO: This number needs to be modified
	...
	for(const auto &shape : shapes) {
		...
		// Use boneTransforms to compute the transformation of each bone
...
void loadSkeleton(vector<vector<BoneTransform>>& boneTransforms) {
	string filename = DATA_DIR + "bigvegas_Walking_skel.txt";
	ifstream in;
	in.open(filename);
	if (!in.good()) {
		cout << "Cannot read " << filename << endl;
	...
	string line;
	int frameCount = 0, boneCount = 0;
	// Skip the first three lines
	for (int i = 0; i < 3; ++i) {
		getline(in, line);
	}
	// Read the number of frames and bones
	getline(in, line);
	istringstream headeriss(line);
	headeriss >> frameCount >> boneCount;

	// Assign the size of the data structure
	boneTransforms.resize(frameCount, vector<BoneTransform>(boneCount));
	boneMatrices.resize(frameCount, vector<glm::mat4>(boneCount));

	// Read bone transforms from file
	for (int frame = 0; frame < frameCount; ++frame) {
		// Read a line of frame, which contains the quaternion and position of each bone
		getline(in, line);
		istringstream transformiss(line);
		for (int bone = 0; bone < boneCount; ++bone) {
			//Each bone has 7 floats, so read 7 floats at a time
			transformiss >> boneTransforms[frame][bone].qx
				>> boneTransforms[frame][bone].qy
				>> boneTransforms[frame][bone].qz
				>> boneTransforms[frame][bone].qw
				>> boneTransforms[frame][bone].px
				>> boneTransforms[frame][bone].py
				>> boneTransforms[frame][bone].pz;
			// Compute the boneMatrix for each bone
			glm::quat q(boneTransforms[frame][bone].qw, boneTransforms[frame][bone].qx, boneTransforms[frame][bone].qy, boneTransforms[frame][bone].qz);
			glm::vec3 p(boneTransforms[frame][bone].px, boneTransforms[frame][bone].py, boneTransforms[frame][bone].pz);
			glm::mat4 M = mat4_cast(q);
			M[3] = glm::vec4(p, 1.0f);
			boneMatrices[frame][bone] = M;
		}
	}
	...
	// Debug: print matrices of the bind pose and the first frame of the first bone
	
	cout << "Bind pose of the first bone: " << endl;
	cout << glm::to_string(boneMatrices[0][0]) << endl;
	cout << "First frame of the first bone: " << endl;
	cout << glm::to_string(boneMatrices[1][0]) << endl;
	cout << "Size of boneMatrices:" << boneMatrices.size() << endl;
	cout << "Size of boneTransforms:" << boneTransforms.size() << endl;
	/*
		Note that this function prints the matrix column by column rather than row by row. 
		In other words, it will display the transposed matrix. 
		Remember that the translation factors should be in the right-most column, not the bottom row.
	*/
}
